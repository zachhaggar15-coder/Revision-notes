...
Set `NEXT_PUBLIC_SITE_URL` to the production domain before deploying, especially when using a custom domain. If it is not set, production metadata falls back to `https://revision-notes-eight.vercel.app`. The public sitemap is a static file at `public/sitemap.xml`.
