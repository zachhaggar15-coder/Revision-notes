...
Set `NEXT_PUBLIC_SITE_URL` to the production domain before deploying, especially when using a custom domain. If it is not set, metadata falls back to Vercel system URL variables and then to `https://revision-notes-eight.vercel.app`. The public sitemap is a static file at `public/sitemap.xml`.
